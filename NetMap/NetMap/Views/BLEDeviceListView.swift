...
struct BLEDeviceListView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // Scan is now managed at ContentView level — just restart if somehow stopped
    ...
    private var listColumnContent: some View {
        Group {
            if bleScanner.bluetoothState == .unknown {
                ...
                // HIG : .inset sur macOS, .plain sur iOS dans NavigationSplitView
                #if os(macOS)
                .listStyle(.inset)
                #else
                .listStyle(.plain)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                // ── Pairing card ───────────────────────────────
                VehiclePairingCard(device: device, showSheet: $showPairSheet)
                    .environmentObject(store)

                // ── Section Michelin TMS ──────────────────────
                if device.isTMSDevice, let tms = device.tmsData {
                    TMSSectionView(tms: tms)
                    Divider().padding(.vertical, 4)
                }

                // ── Section STIHL Smart Connector ──────────────
                if let connector = device.stihlConnectorData {
                    StihlConnectorSectionView(data: connector)
                    Divider().padding(.vertical, 4)
                }

                // ── Section STIHL Smart Battery ─────────────────
                if let battery = device.stihlBatteryData {
                    StihlBatterySectionView(data: battery)
                    Divider().padding(.vertical, 4)
                }

                // ── Section ELA Innovation ──────────────────────
                if let ela = device.elaData {
                ...
                BLEInfoSection(title: "Signal") {
                    ...
                    if let tx = device.txPowerLevel {
                        BLEInfoRow(label: "TX Power", value: "\(tx) dBm")
                    }
                    if let dist = device.estimatedDistance {
                ...
                BLEInfoSection(title: "Activity") {
                    ...
                    BLEInfoRow(label: "Last seen",
                               value: device.lastSeen.formatted(.relative(presentation: .named)))
                }
                ...
                HStack {
                    Spacer()
                    Button {
                        copyToClipboard(device.id.uuidString)
                    } label: {
                        Label("Copy UUID", systemImage: "doc.on.clipboard")
                            .font(.caption)
                    }
