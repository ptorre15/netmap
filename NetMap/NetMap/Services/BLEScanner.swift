...
extension BLEScanner: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, willRestoreState dict: [String: Any]) {
        // If iOS is restoring a scan that was running before termination, mark it active.
    ...
    func centralManager(
        ...
        if let idx = devices.firstIndex(where: { $0.id == peripheral.identifier }) {
            ...
            if let sc = stihlConn { devices[idx].stihlConnectorData = sc }
            if let sb = stihlBatt { devices[idx].stihlBatteryData   = sb }
            // Re-decode ELA
            devices[idx].elaData = parseELAData(
                rawManufacturerData: mfRaw,
                deviceName: devices[idx].name
            )
        } else {
