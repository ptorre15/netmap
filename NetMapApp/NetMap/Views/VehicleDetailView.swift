...
struct VehicleDetailView: View {
    ...
    private func content(_ v: VehicleConfig) -> some View {
        List {
            ...
            Section {
                if v.pairedSensors.isEmpty {
                    ...
                    ForEach(v.pairedSensors) { sensor in
                        let live: BLEDevice? = {
                            // AirTag: UUID is ephemeral (rotates by Apple design, nil on iOS for owner's
                            // own tags). Never use it — always match by advertisement name or, on iOS
                            // where name is hidden, accept the sole anonymous AirTag in range.
                            if sensor.brand == .airtag {
                                let storedName = sensor.macAddress ?? ""
                                if !storedName.isEmpty,
                                   let d = bleScanner.devices.first(where: { $0.isAirTagDevice && $0.name == storedName }) {
                                    return d
                                }
                                // iOS fallback: peripheral.name is nil for owner's AirTags
                                let anon = bleScanner.devices.filter { $0.isAirTagDevice && $0.name == nil }
                                return anon.count == 1 ? anon[0] : nil
                            }
                            // Primary: CBPeripheral UUID (stable within an install)
                            if let d = bleScanner.devices.first(where: { $0.id == sensor.id }) { return d }
                            // Fallback for Stihl — UUID may differ after server sync or reinstall
                            if sensor.brand == .stihl, let mac = sensor.macAddress {
                                if mac.hasPrefix("STIHLBATT-") {
                                    let serial = String(mac.dropFirst(10))
                                    return bleScanner.devices.first { $0.stihlBatteryData?.serialNumber == serial }
                                }
                                return bleScanner.devices.first { $0.stihlConnectorData?.macAddress == mac }
                            }
                            return nil
                        }()
                        PairedSensorRow(
                            ...
                            onUnpair: {
                                ...
                                let sid = sensor.brand == .airtag ? (sensor.macAddress ?? sensor.id.uuidString) : sensor.id.uuidString
            ...
            // ── Tires map section (when sensors have wheel positions) ─
...
struct PairedSensorRow: View {
    ...
    private var innerContent: some View {
            VStack(alignment: .leading, spacing: 2) {
                ...
                } else if let at = liveDevice?.airtagData {
                    HStack(spacing: 6) {
                        Image(systemName: at.batteryLevel.systemImage)
                            .foregroundStyle(at.batteryLevel.color)
                        Text(at.batteryLevel.label)
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(at.batteryLevel.color)
                        if at.isSeparated {
...
struct PairSensorSheet: View {
    ...
    private func deviceMatchesBrand(_ d: BLEDevice) -> Bool {
        ...
        case .airtag:   return d.isAirTagDevice
        case .tracker:  return false   // GPS trackers are not BLE — paired via web only
    ...
    private var isSaveEnabled: Bool {
        ...
        // AirTag on iOS: name is hidden — require a custom label to produce a stable key
        if selectedBrand == .airtag,
           bleScanner.devices.first(where: { $0.id == selectedDeviceID })?.name == nil,
           customLabel.isEmpty { return false }
        return true
    }
    ...
    private func pair() {
        ...
        let hardwareKey: String? = {
            ...
            if let sb = pairedDevice?.stihlBatteryData   { return "STIHLBATT-\(sb.serialNumber)" }
            if selectedBrand == .airtag {
                // On iOS, peripheral.name is nil for owner's AirTags — fall back to the
                // user-supplied label so the sensor gets a stable macAddress key.
                return pairedDevice?.name ?? (customLabel.isEmpty ? nil : customLabel)
            }
            return nil
        }()
        let sensor = PairedSensor(
        ...
        // Push pairing to server immediately via dedicated API (no BLE readings required).
        // Do NOT gate on isEnabled — pairing is an explicit user action that must always
        // be registered on the server if it is reachable, regardless of the BLE push toggle.
        if let vehicle = store.vehicles.first(where: { $0.id == vehicleID }),
           !serverClient.host.isEmpty {
            ...
            let sensorID: String = {
                if let sc = pairedDevice?.stihlConnectorData {
                    return "STIHL-" + sc.macAddress.replacingOccurrences(of: ":", with: "")
                }
                if let sb = pairedDevice?.stihlBatteryData {
                    return "STIHLBATT-\(sb.serialNumber)"
                }
                // AirTag: use the device name (= sensor.macAddress) as stable ID,
                // matching SensorPushService so push and pair share the same server key.
                if selectedBrand == .airtag, let name = sensor.macAddress, !name.isEmpty {
                    return name
                }
                return pairedDevice?.stableSensorID ?? sensor.id.uuidString
            }()
            let payload = NetMapServerClient.PairingPayload(
                ...
                sensorName:        sensor.customLabel ?? (pairedDevice?.displayName == "Unknown" ? nil : pairedDevice?.displayName)
            )
...
struct EditPairedSensorSheet: View {
    ...
    private func save() {
        ...
        if let vehicle = store.vehicles.first(where: { $0.id == vehicleID }),
           !serverClient.host.isEmpty {
            let vid = vehicle.serverVehicleID?.uuidString ?? vehicle.id.uuidString
            let liveDevice = bleScanner.devices.first { $0.id == sensor.id }
            let sensorID: String = {
                ...
                return liveDevice?.stableSensorID ?? sensor.stableSensorID
            }()
            let payload = NetMapServerClient.PairingPayload(
                sensorID:          sensorID,
                vehicleID:         vid,
                vehicleName:       vehicle.name,
                assetTypeID:       vehicle.assetTypeID,
                brand:             updated.brand.rawValue,
                wheelPosition:     updated.wheelPosition?.rawValue,
                targetPressureBar: updated.targetPressureBar,
                sensorName:        updated.customLabel
            )
            Task { try? await serverClient.pushPairing(payload) }
        }
...
struct SensorDetailView: View {
    ...
    private var liveDevice: BLEDevice? {
        // Primary match: CBPeripheral UUID
        if let dev = bleScanner.devices.first(where: { $0.id == sensor.id }) { return dev }
        // Fallback for Stihl — UUID may differ after server sync or app reinstall
        if sensor.brand == .stihl, let mac = sensor.macAddress {
            if mac.hasPrefix("STIHLBATT-") {
                let serial = String(mac.dropFirst(10))
                return bleScanner.devices.first { $0.stihlBatteryData?.serialNumber == serial }
        ...
        // Fallback for AirTag — CBPeripheral UUID differs between macOS and iOS
        // for the same physical device. At pairing time, macAddress is set to
        // the AirTag's advertisement name, which IS stable.  Match on that.
        if sensor.brand == .airtag, let storedName = sensor.macAddress, !storedName.isEmpty {
            return bleScanner.devices.first { $0.isAirTagDevice && $0.name == storedName }
        }
        return nil
    }

    /// Last known Stihl Connector data — live if in range, cached otherwise.
    private var stihlConnector: StihlConnectorData? {
        liveDevice?.stihlConnectorData ?? bleScanner.lastKnownStihlConnector[sensor.id]
    }
    /// Last known Stihl Battery data — live if in range, cached otherwise.
    private var stihlBattery: StihlBatteryData? {
        liveDevice?.stihlBatteryData ?? bleScanner.lastKnownStihlBattery[sensor.id]
    }
    /// Last known AirTag data — live if in range, cached otherwise.
    private var airtagData: AirTagData? {
        liveDevice?.airtagData ?? bleScanner.lastKnownAirTagData[sensor.id]
    }
    /// Last known ELA data — live if in range, cached otherwise.
    private var elaData: ELAData? {
        liveDevice?.elaData ?? bleScanner.lastKnownELAData[sensor.id]
    }
    /// Latest server stats for GPS trackers (keyed by IMEI stored in macAddress).
    private var trackerStat: SensorServerDTO? {
        guard sensor.brand == .tracker, let imei = sensor.macAddress else { return nil }
        return store.latestServerStats[imei]
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let sc = stihlConnector {
                    if liveDevice?.stihlConnectorData == nil {
                        Label("Last known data — sensor out of range", systemImage: "clock.arrow.circlepath")
                ...
                if let sb = stihlBattery {
                    if liveDevice?.stihlBatteryData == nil {
                        Label("Last known data — sensor out of range", systemImage: "clock.arrow.circlepath")
                            .font(.caption).foregroundStyle(.secondary)
                            .padding(.bottom, 2)
                    }
                ...
                if let at = airtagData {
                    if liveDevice?.airtagData == nil {
                        Label("Last known data — sensor out of range", systemImage: "clock.arrow.circlepath")
                            .font(.caption).foregroundStyle(.secondary)
                            .padding(.bottom, 2)
                    }
                    AirTagSectionView(data: at, estimatedDistance: liveDevice?.estimatedDistance)
                    Divider().padding(.vertical, 4)
                }

                // ── Tracker (server-side telemetry) ───────────────────
                if let stat = trackerStat {
                    TrackerSectionView(stat: stat)
                    Divider().padding(.vertical, 4)
                }
                ...
                if let dev = liveDevice {
                    BLEInfoSection(title: "Signal") {
                        ...
                        if let tx = dev.txPowerLevel {
                            BLEInfoRow(label: "TX Power", value: "\(tx) dBm")
                        }
                    ...
                    BLEInfoSection(title: "Identification") {
                        BLEInfoRow(label: "UUID", value: dev.id.uuidString)
                        BLEInfoRow(label: "Connectable", value: dev.isConnectable ? "Yes" : "No")
                        if let data = dev.manufacturerData {
                            BLEInfoRow(label: "Mfr. Data",
                    ...
                    BLEInfoSection(title: "Activity") {
                        BLEInfoRow(label: "Packets", value: "\(dev.seenCount)")
                ...
                BLEInfoSection(title: "Configuration") {
                    ...
                    // Show UUID only when device is not live (already in Identification when live)
                    if liveDevice == nil {
        ...
        // Heal AirTag UUID: CBPeripheral UUID differs per-platform (macOS vs iOS).
        // When matched by name, persist the current UUID so future scans hit the fast path.
        .task(id: liveDevice?.id) {
            if sensor.brand == .airtag,
               let d = liveDevice, d.id != sensor.id,
               let name = sensor.macAddress, !name.isEmpty {
                store.healSensorUUID(fromMAC: name, to: d.id, in: vehicleID)
            }
        }
    }
}
...
/// Tracker telemetry section — uses latest server-cached data for GPS trackers.
@MainActor
struct TrackerSectionView: View {
    let stat: SensorServerDTO

    private var coordinateText: String? {
        guard let lat = stat.latestLatitude, let lon = stat.latestLongitude else { return nil }
        return String(format: "%.5f, %.5f", lat, lon)
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            // "Last known" banner when data is older than 30 minutes
            let isStale: Bool = {
                guard let ts = stat.latestTimestamp else { return true }
                return Date().timeIntervalSince(ts) > 1800
            }()
            if isStale {
                Label("Last known data — tracker may be offline", systemImage: "clock.arrow.circlepath")
                    .font(.caption).foregroundStyle(.secondary)
                    .padding(.bottom, 2)
            }

            BLEInfoSection(title: "Location") {
                if let coords = coordinateText {
                    // Tappable coordinates → open in Maps
                    if let lat = stat.latestLatitude, let lon = stat.latestLongitude,
                       let url = URL(string: "maps://?ll=\(lat),\(lon)&q=Tracker") {
                        Link(destination: url) {
                            HStack {
                                Text("Coordinates")
                                    .font(.caption).foregroundStyle(.secondary)
                                    .frame(width: 90, alignment: .leading)
                                Label(coords, systemImage: "map")
                                    .font(.caption.monospaced())
                                    .foregroundStyle(.blue)
                            }
                        }
                    } else {
                        BLEInfoRow(label: "Coordinates", value: coords)
                    }
                } else {
                    BLEInfoRow(label: "Coordinates", value: "–")
                }
                if let sats = stat.latestGpsSatellites {
                    BLEInfoRow(label: "Satellites", value: "\(sats)")
                }
                if let ts = stat.latestTimestamp {
                    BLEInfoRow(label: "Last fix",
                               value: ts.formatted(.relative(presentation: .named)))
                }
            }

            if stat.latestBatteryPct != nil || stat.latestTemperatureC != nil {
                BLEInfoSection(title: "Telemetry") {
                    if let fuel = stat.latestBatteryPct {
                        BLEInfoRow(label: "Fuel", value: "\(fuel)%")
                    }
                    if let temp = stat.latestTemperatureC {
            ...
            if let count = stat.readingCount {
                BLEInfoSection(title: "Activity") {
                    BLEInfoRow(label: "Records", value: "\(count)")
                    if let ts = stat.latestTimestamp {
                        BLEInfoRow(label: "Last seen",
                                   value: ts.formatted(.relative(presentation: .named)))
                    }
