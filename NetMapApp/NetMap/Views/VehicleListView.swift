...
struct VehicleListView: View {
    ...
    // Assets are created and edited exclusively from the web dashboard (server is master).
    ...
    var body: some View {
        ...
        NavigationStack {
            listContent
                ...
                .onAppear {
                    guard !serverClient.host.isEmpty else { return }
                    Task { await syncFromServer() }
                }
                .refreshable { await syncFromServer() }
        }
        #else
        NavigationSplitView {
            listContent
                ...
                .toolbar {
                    ToolbarItem(placement: .automatic) {
                        ...
                        .disabled(isRefreshing)
                        .help("Refresh assets from server")
                    }
                }
                .onAppear {
                    guard !serverClient.host.isEmpty else { return }
    ...
    private var listContent: some View {
        if store.vehicles.isEmpty {
            ContentUnavailableView {
                ...
                Text("Add assets from the web dashboard.")
            }
        } else {
            List {
    ...
    private func syncFromServer() async {
        ...
        if let types   { store.mergeServerAssetTypes(types) }
        if let assets  { store.syncFromServer(assets) }
        if let sensors { store.syncPairedSensorsFromServer(sensors) }
    }
