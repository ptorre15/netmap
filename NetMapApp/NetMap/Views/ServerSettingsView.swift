...
struct ServerSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("When enabled, every TMS sensor reading is forwarded to the NetMapServer in real-time.")
            }

            // ── Address ──────────────────────────────────────────────────
            Section {
                Toggle(isOn: $serverClient.useHTTPS) {
                    Label(serverClient.useHTTPS ? "HTTPS (secure)" : "HTTP (unsecure)",
                          systemImage: serverClient.useHTTPS ? "lock.fill" : "lock.open")
                }
                ...
                HStack {
                    Label("Port", systemImage: "number")
                        .frame(width: 76, alignment: .leading)
                    TextField(serverClient.useHTTPS ? "443" : "8092", text: $portDraft)
                    #if os(iOS)
            ...
            // ── Push interval ────────────────────────────────────────────
            Section {
                Picker(selection: $serverClient.pushIntervalSeconds) {
                    Text("1 min").tag(60)
                    Text("2 min").tag(120)
                    Text("5 min").tag(300)
                    Text("10 min").tag(600)
                    ...
                    Label("Push interval", systemImage: "clock.arrow.2.circlepath")
                }
            } header: {
                Text("Data Forwarding")
            } footer: {
                Text("How often each sensor's data is sent to the server. Lower = more data, higher battery use.")
            }
            ...
            Section {
                Toggle(isOn: Binding(
                    ...
                    set: { if $0 { launchAgent.install() } else { launchAgent.uninstall() } }
                )) {
                    Label("Start at login", systemImage: "power")
                }
                if let err = launchAgent.errorMessage {
                ...
                Text("Installs a LaunchAgent so NetMap launches automatically when you log in.")
            }
            #endif
            ...
            Section {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text("Default: HTTPS on port 443 via Caddy reverse proxy. HTTP fallback: port **8092** — override with `PORT=xxxx swift run App`")
    ...
    private func applyDraft() {
        ...
        let portStr = portDraft.trimmingCharacters(in: .whitespaces)
        if portStr.isEmpty {
            serverClient.port = serverClient.useHTTPS ? 443 : 8092
        } else if let p = Int(portStr), p > 0, p < 65536 {
            serverClient.port = p
        }
